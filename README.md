...
- **매일 23:45(23:46 트리거) 실행에서 누적 랭킹 18세트** — 금액순(T)·건수순(Q) 각각 카테고리별(종합·뷰티·식품)+연령대별(전연령·10~50대) 1~100위 CSV 저장(`랭킹종류`=금액순|건수순, `카테고리` 컬럼 구분). 스크린샷은 금액순만: 카테고리별 3종 매일, 연령대별 6종은 이벤트 마지막날 1회 (`ranking_amount_<세트>_<날짜>.jpg`). 순위 파싱·스크린샷을 상세 보강보다 먼저 수행해 자정 전에 확보(보강은 자정 컷오프)
- **매일 23:02 (cron-job.org)** — `mode=prewarm` 캐시 예열: 금액순 상품 상세를 자정 전에 미리 캐시에 적재해 23:46 본 수집이 자정 전에 완결되도록 함 (CSV·스크린샷 저장 없음. 상세 보강은 자정 컷오프 — 날짜가 바뀌면 중단해 오염 방지)
- **매일 09:30** — 이벤트 메인 페이지 전체 스크린샷 → `data/screenshots/megapo_main_*.jpg` (주 트리거: cron-job.org 09:31 JST / 백업: GitHub cron)
- **워치리스트** — `watch_list.txt`의 [자사]/[계열] 브랜드(`키: 토큰,토큰` 형식, 브랜드·점포명 부분일치)가 랭킹에 매칭되면 '자사상품여부' 컬럼에 **자사/계열** 기록 + **하루 최고 순위 갱신 시점 스크린샷을 브랜드당 1장**(`watch_<키>_<날짜>.jpg` 덮어쓰기)

샵·브랜드·가격 상세는 상품 페이지를 방문해 수집 (상품별 3시간 캐시 `data/product_cache.json`).

## 매 이벤트 운영 (매달 URL 변경됨)

1. `event_config.txt`에서 기간(period)·event_name·랭킹 URL·이벤트 URL 네 줄 수정 (시작 전날까지 — 기간 밖이면 미리 넣어둬도 안전). period에는 시각(HH:MM, JST)도 가능 — 예: `2026-08-28 17:00 ~ 2026-09-09` (메가와리는 전달 말일 17:00 개시)
