...
- **매시간 :45** — 리얼타임 랭킹 1~100위 → `data/cumulative.csv` 누적 (저장일/저장시각/순위/상품코드/상품명/최종가/점포/브랜드/참고가·판매가·타임세일가/쿠폰할인율/포인트/자사상품여부/순위변동). 원본 HTML은 `data/html/*.html.gz`
- **매시간 :15 (백업)** — 직전 수집이 75분 이상 지났을 때만 랭킹 수집 (GitHub cron 누락 보완, 평상시엔 수십 초 만에 skip 종료)
...
- **워크플로 파일 push·Enable API 호출 등 "스케줄 재등록" 이벤트 후 약 6시간 cron이 발동하지 않는 패턴 관측** (2026-07-08). 같은 이유로 매 실행마다 enable을 호출하던 keepalive 단계를 제거함 — 이벤트 첫날·워크플로 수정 직후에는 수동 실행(Run workflow)이나 로컬 백업 트리거로 보완할 것
