...
- **매일 23시 실행에서 누적 랭킹 18세트** — 금액순(T)·건수순(Q) 각각 카테고리별(종합·뷰티·식품)+연령대별(전연령·10~50대) 1~100위 CSV 저장(`랭킹종류`=금액순|건수순, `카테고리` 컬럼 구분). 스크린샷은 금액순만: 카테고리별 3종 매일, 연령대별 6종은 이벤트 마지막날 1회 (`ranking_amount_<세트>_<날짜>.jpg`)
